...
from CourseFunctions.lab_utils_common import np, plt, dlblue, dlorange, sigmoid, dldarkred, gradient_descent
...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        ...
        # Get the height of the label bounding box
        h = bcid.labels[0].get_window_extent().height

        # Adjust the size of the label's bounding box
        bcid.labels[0].set_bbox(dict(facecolor='white', edgecolor='black', boxstyle='square,pad=0.3'))

        # Optionally, set the height of the rectangle or perform further adjustments
        # If you need to set the height of the checkbox's rectangle, you can access it like this
        # (if the rectangle attribute is present, depending on your Matplotlib version):
